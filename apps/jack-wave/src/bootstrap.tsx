 ...
 * 调用 mount(container, { player, onError }) 把应用渲染进外壳容器，
 * 卸载时调用 unmount()。应用不会创建自己的 <audio>（使用外壳传入的 player），
 * 因此导航切换时全局播放不中断。
 */

import { createRoot, type Root } from 'react-dom/client';
import { WaveAppEmbedded } from './App';
...
export function mount(container: HTMLElement, props: EmbedProps): void {
  ...
  // 这里显式指回本应用的静态资源 / Functions 路径，避免 avatar、/api/* 404。
  setAssetBase(props.assetBase ?? EMBED_ASSET_BASE);
  root = createRoot(container);
  root.render(<WaveAppEmbedded player={props.player} />);
}
